...
var i = class _i {
  static async create({ sampleRate: e2, format: t2, preferHeadphonesForIosDevices: n2 }) {
    ...
    try {
      ...
      c3 || await s2.audioWorklet.addModule("https://cdn.jsdelivr.net/npm/@alexanderolsen/libsamplerate-js@2.1.2/dist/libsamplerate.worklet.js"), await s2.audioWorklet.addModule(a), r2 = await navigator.mediaDevices.getUserMedia({ audio: l3 });
      const d2 = s2.createMediaStreamSource(r2), h2 = new AudioWorkletNode(s2, "raw-audio-processor");
      return h2.port.postMessage({ type: "setFormat", format: t2, sampleRate: e2 }), d2.connect(u2), u2.connect(h2), await s2.resume(), new _i(s2, u2, h2, r2);
    } catch (e3) {
      var l2, c2;
      throw null == (l2 = r2) || l2.getTracks().forEach((e4) => e4.stop()), null == (c2 = s2) || c2.close(), e3;
    }
  }
...
var c = class _c {
  static async create({ sampleRate: e2, format: t2 }) {
    ...
    try {
      ...
      a2.connect(s3), s3.connect(n2.destination), await n2.audioWorklet.addModule(l);
...
var f = class _f {
  static async startSession(t2) {
    ...
    let s2 = null, a2 = null, r2 = null, l2 = null;
    try {
      var u2, h2;
      ...
      return m2 > 0 && await new Promise((e2) => setTimeout(e2, m2)), a2 = await d.create(t2), [s2, r2] = await Promise.all([i.create(e({}, a2.inputFormat, { preferHeadphonesForIosDevices: t2.preferHeadphonesForIosDevices })), c.create(a2.outputFormat)]), null == (h2 = l2) || h2.getTracks().forEach((e2) => e2.stop()), l2 = null, new _f(n2, a2, s2, r2);
    } catch (e2) {
      var _, y, w, b;
      throw n2.onStatusChange({ status: "disconnected" }), null == (_ = l2) || _.getTracks().forEach((e3) => e3.stop()), null == (y = a2) || y.close(), await (null == (w = s2) ? void 0 : w.close()), await (null == (b = r2) ? void 0 : b.close()), e2;
    }
  }
  constructor(e2, s2, a2, o2) {
    ...
    this.options = void 0, this.connection = void 0, this.input = void 0, this.output = void 0, this.lastInterruptTimestamp = 0, this.mode = "listening", this.status = "connecting", this.inputFrequencyData = void 0, this.outputFrequencyData = void 0, this.volume = 1, this.currentEventId = 1, this.lastFeedbackEventId = 1, this.canSendFeedback = false, this.endSession = () => this.endSessionWithDetails({ reason: "user" }), this.endSessionWithDetails = async function(e3) {
      ...
      switch (e3.type) {
        case "interruption":
          return e3.interruption_event && (i2.lastInterruptTimestamp = e3.interruption_event.event_id), void i2.fadeOutAudio();
        case "agent_response":
          return void i2.options.onMessage({ source: "ai", message: e3.agent_response_event.agent_response });
        case "user_transcript":
          return void i2.options.onMessage({ source: "user", message: e3.user_transcription_event.user_transcript });
        case "internal_tentative_agent_response":
          return void i2.options.onDebug({ type: "tentative_agent_response", response: e3.tentative_agent_response_internal_event.tentative_agent_response });
        case "client_tool_call":
          if (console.info("Received client tool call request", e3.client_tool_call), i2.options.clientTools.hasOwnProperty(e3.client_tool_call.tool_name)) try {
            var t2;
            const n2 = null != (t2 = await i2.options.clientTools[e3.client_tool_call.tool_name](e3.client_tool_call.parameters)) ? t2 : "Client tool execution successful.", s3 = "object" == typeof n2 ? JSON.stringify(n2) : String(n2);
            i2.connection.sendMessage({ type: "client_tool_result", tool_call_id: e3.client_tool_call.tool_call_id, result: s3, is_error: false });
          } catch (t3) {
            ...
            if (i2.options.onUnhandledClientToolCall) return void i2.options.onUnhandledClientToolCall(e3.client_tool_call);
          ...
          return;
        case "audio":
          return void (i2.lastInterruptTimestamp <= e3.audio_event.event_id && (i2.options.onAudio(e3.audio_event.audio_base_64), i2.addAudioBase64Chunk(e3.audio_event.audio_base_64), i2.currentEventId = e3.audio_event.event_id, i2.updateCanSendFeedback(), i2.updateMode("speaking")));
        case "ping":
          return void i2.connection.sendMessage({ type: "pong", event_id: e3.ping_event.event_id });
        default:
          return void i2.options.onDebug(e3);
      }
